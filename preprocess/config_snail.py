...
def export_parameters(args):
    ...
    GROUP = '{}n{}s_xy{}{}{}'.format(NSWEEP, AVA_SEQ_LEN, PSEUDO_Z, FEATURE, '_remove_' + args.data_type if REMOVE else '')
    STRUCT_DIR = os.path.join(args.dataset_dir, GROUP)
    IMG_DIR = os.path.join(STRUCT_DIR, 'img')
    RCS_DIR = os.path.join(STRUCT_DIR, 'rcs')
    PCL_DIR = os.path.join(args.dataset_dir, 'processed_snail_radar', args.data_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_dir", required=True, type=str, default='/LOCAL/ramdrop/dataset/mmrec_dataset')
    parser.add_argument("--data_type", required=True, type=str, default='ars548')
